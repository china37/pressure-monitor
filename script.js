...
// โชว์ tooltip ตามตำแหน่งเมาส์
  ...
  function generateWavePressure(base, amplitude, speed) {
    const wave = Math.sin(timeCounter * speed);
    const value = base + amplitude * wave;
    return Math.round(value * 10) / 10;
  }

  setInterval(() => {
    timeCounter += 1; // เพิ่มเวลา

    const timeNow = Date.now();

    simulatedPressure.KneeLeft = generateWavePressure(50, 20, 0.1);
    simulatedPressure.Bottom = generateWavePressure(60, 15, 0.12);
    simulatedPressure.KneeRight = generateWavePressure(55, 18, 0.08);

  // เขียนค่าไป Firebase
    firebase.database().ref("pressure").set(simulatedPressure);
    ...
    console.log("Simulated:", simulatedPressure);
  }, 1000);
