...
// 🔄 ดึงข้อมูลแบบ real-time
  ...
  function updateSensorValues(data) {
    ...
    const now = new Date();
    chartKneeLeft.data.datasets[0].data.push({ x: now, y: leftValue });
    chartBottom.data.datasets[0].data.push({ x: now, y: middleValue });
    chartKneeRight.data.datasets[0].data.push({ x: now, y: rightValue });

    function getMinTime(dataset) {
      if (!dataset.length) return now;
    ...
    chartKneeLeft.options.scales.x.min = minTime;
    chartKneeLeft.options.scales.x.max = maxTime;
    chartBottom.options.scales.x.min = minTime;
    chartBottom.options.scales.x.max = maxTime;
    chartKneeRight.options.scales.x.min = minTime;
    chartKneeRight.options.scales.x.max = maxTime;
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: {
              autoskip: false,
              maxRotation: 90, // ✅ หมุน label 90 องศา
              minRotation: 45,
              source: 'linear',
