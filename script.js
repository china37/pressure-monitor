...
// ฟังก์ชันแปลงค่าความดันเป็นสี
  ...
  function classifyAndCountPressure(value) {
    if (value == null || value === 0) {
      pressureCounters.gray++;
    } else if (value > 0 && value <= 31.9) {
      pressureCounters.green++;
    } else if (value > 31.9 && value <= 70) {
      pressureCounters.yellow++;
    } else if (value > 70) {
      pressureCounters.red++;
    }
    updateCountDisplay();
  }

  function updateCountDisplay() {
    document.getElementById("count-gray").textContent = pressureCounters.gray;
    document.getElementById("count-green").textContent = pressureCounters.green;
    document.getElementById("count-yellow").textContent = pressureCounters.yellow;
    document.getElementById("count-red").textContent = pressureCounters.red;
  }

  function resetCounters() {
    pressureCounters.gray = 0;
    pressureCounters.green = 0;
    pressureCounters.yellow = 0;
    pressureCounters.red = 0;
    updateCountDisplay();
  }
...
// 🔄 ดึงข้อมูลแบบ real-time
  database.ref('pressure').on('value', function(snapshot) {
    ...
    database.ref('pressure').once('value').then(snap => {
      ...
      resetCounters();  // Reset counters before classifying new values

      classifyAndCountPressure(leftValue);
      classifyAndCountPressure(middleValue);
      classifyAndCountPressure(rightValue);
