...
// 🔄 ดึงข้อมูลแบบ real-time
  ...
  // ตรวจสอบว่ามีค่าความดันในแต่ละเซ็นเซอร์หรือไม่
      ...
      const now = new Date();
      chartKneeLeft.data.datasets[0].data.push({ x: now, y: formatPressure(leftValue) });
      chartBottom.data.datasets[0].data.push({ x: now, y: formatPressure(middleValue) });
      chartKneeRight.data.datasets[0].data.push({ x: now, y: formatPressure(rightValue) });
