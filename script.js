...
function generateInitialTimestamps() {
  ...
  for (let i = 0; i <= 60; i += 10) {
    timestamps.push(new Date(oneHourAgo + i * 60 * 1000));
  }
  return timestamps;
}
...
function loadHistoryAndPlotChart() {
  ...
  database.ref('history')
    ...
    .then(snapshot => {
      ...
      chartKneeLeft.data.labels = labels;
      chartKneeLeft.data.datasets[0].data = left;

      chartBottom.data.labels = labels;
      chartBottom.data.datasets[0].data = middle;

      chartKneeRight.data.labels = labels;
      chartKneeRight.data.datasets[0].data = right;
