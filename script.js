  ...
  function loadAndListenHistory() {
    const historyRef = firebase.database().ref("history");
      ...
      function updateCount(sensor, value) {
        if (value > 31.9 && value <= 70) pressureCounts[sensor].yellow++;
        else if (value > 70) pressureCounts[sensor].red++;

        document.getElementById('count-${sensor.toLowerCase()}-yellow').textContent = pressureCounts[sensor].yellow;
        document.getElementById('count-${sensor.toLowerCase()}-red').textContent = pressureCounts[sensor].red;
      }
