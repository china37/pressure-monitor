...
// ✅ ดึงข้อมูลจาก Firebase ทุกครั้งที่มีการเปลี่ยนแปลง
  ...
  function setChartMinWidth(canvasId, labelsCount) {
    const pxPerPoint = 50;
    const minWidth = labelsCount * pxPerPoint;

    const canvas = document.getElementById(canvasId);
    const wrapper = canvas.closest('.chart-wrapper');

    canvas.style.minWidth = `${minWidth}px`;
    wrapper.style.minWidth = `${minWidth + 100}px`;
  }
  setChartMinWidth('chartKneeLeft', data.labels.length);
  setChartMinWidth('chartBottom', data.labels.length);
  setChartMinWidth('chartKneeRight', data.labels.length);
        ...
        scales: {
          x: {
            ...
            ticks: {
              ...
              source: 'linear',
              autoSkip: false,
              maxTicksLimit: 96, // แสดงสูงสุด 24 ชั่วโมง
