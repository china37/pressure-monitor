  ...
  function loadAndListenHistory() {
    const historyRef = firebase.database().ref("history");
      ...
      historyRef.once('value').then(snapshot => {
        ...
        Object.entries(data).forEach(([_, entry]) => {
          ...
          if (entry.KneeLeft !== undefined) {
            const mmHg = convertRawToMmHg(entry.KneeLeft);
            console.log('KneeLeft mmHg =', mmHg); 
            allKneeLeft.push({ x: timestamp, y: mmHg });
          ...
          if (entry.KneeRight !== undefined) {
            const mmHg = convertRawToMmHg(entry.KneeRight);
            console.log('KneeRight mmHg =', mmHg); 
            allKneeRight.push({ x: timestamp, y: mmHg });
      ...
      historyRef.on('child_added', snapshot => {
        ...
        console.log('🧮 Converted', { leftValue, middleValue, rightValue });
