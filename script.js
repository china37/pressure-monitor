  ...
  function loadAndListenHistory() {
    const historyRef = firebase.database().ref("history");
      ...
      historyRef.once('value').then(snapshot => {
        ...
        filterEvery10Seconds(allKneeLeft).forEach(p => addPointToChart('KneeLeft', p.y, new Date(p.x)));
        filterEvery10Seconds(allBottom).forEach(p => addPointToChart('Bottom', p.y, new Date(p.x)));
        filterEvery10Seconds(allKneeRight).forEach(p => addPointToChart('KneeRight', p.y, new Date(p.x)));
