  ...
  function loadAndListenHistory() {
    const historyRef = firebase.database().ref("history");
      ...
      function addPoint(sensor, value, time) {
        console.log('[addPoint] ${sensor} @ ${time.toLocaleTimeString()} = ${value}');
      ...
      historyRef.once('value').then(snapshot => {
        ...
        filterEvery10Seconds(allKneeLeft).forEach(p => addPoint('KneeLeft', p.y, new Date(p.x)));
        filterEvery10Seconds(allBottom).forEach(p => addPoint('Bottom', p.y, new Date(p.x)));
        filterEvery10Seconds(allKneeRight).forEach(p => addPoint('KneeRight', p.y, new Date(p.x)));
