    ...
    historyRef.once('value').then(snapshot => {
      ...
      Object.entries(data).forEach(([_, entry]) => {
          ...
          if (time >= currenHour && time <= endTime) {
            if (entry.KneeLeft !== undefined) allKneeLeft.push({ y: time.getTime(), x: entry.KneeLeft });
            if (entry.Bottom !== undefined) allBottom.push({ y: time.getTime(), x: entry.Bottom });
            if (entry.KneeRight !== undefined) allKneeRight.push({ y: time.getTime(), x: entry.KneeRight });
          }
      });
