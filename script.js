...
function getColor(value) {
  if (value === undefined || value === null ) return 'gray';  // ถ้าไม่มีค่า ให้แสดงสีเทา
...
function updateSensorValues(data) {
  ...
  const leftValue = (leftValueRaw * 31.25 / 4095).toFixed(2);
  const middleValue = (middleValueRaw * 31.25 / 4095).toFixed(2);
  const rightValue = (rightValueRaw * 31.25 / 4095).toFixed(2);
...
heatmapContainer.addEventListener("mousemove", function (e) {
  ...
  if (closest) {
    ...
    const pressureMMHg = (closest.value * 31.25 / 4095).toFixed(2); // Convert to mmHg
