import { getDatabase, ref, onValue } from "https://www.gstatic.com/firebasejs/9.6.1/firebase-database-compat.js";
...
// ✅ ดึงข้อมูลจาก Firebase ทุกครั้งที่มีการเปลี่ยนแปลง
  ...
  function animateChart(chart, firebaseData) {
    if (!firebaseData.length) return;

    firebaseData.sort((a, b) => a.time - b.time); // เรียงข้อมูลตามเวลา
    const firstTime = firebaseData[0].time;
    const dummyPoints = {
      x: new Date(firstTime - 1000),
      y: null
    };

    chart.data.datasets[0].data = [dummyPoints]; // เพิ่มจุด dummy ก่อนเริ่มแอนิเมชัน
    chart.update();

    let i = 0;
    function addNextPoint() {
      if (i < firebaseData.length) {
        const point = firebaseData[i];
        ...
        chart.update();
        i++;
        setTimeout(addNextPoint, 10000); // เพิ่มจุดถัดไปทุก 100 มิลลิวินาที
      }
    }
    addNextPoint();
  }

  const chartRef = firebase.database().ref('history');
  onValue(chartRef, (snapshot) => {
    ...
    animateChart(myChartInstance, firebaseData);
  });
