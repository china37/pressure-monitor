...
// ✅ ดึงข้อมูลจาก Firebase ทุกครั้งที่มีการเปลี่ยนแปลง
  ...
  function createChart(ctx, label, color, fullHourlyLabels, sensorData) {
    const pressureData = fullHourlyLabels.map((labelTime) => {
      const found = sensorData.find(d =>
        new Date(d.x).getHours() === new Date(labelTime).getHours()
      );
