...
// วาดจุด
  ...
  function saveToHistory(left, middle, right) {
    const key = getRoundedKeyForHistory();  // ปัดเฉพาะ key
    const actualTimestamp = Date.now();     // เก็บเวลาจริงในตัวข้อมูล

    database.ref('history/' + key).update({
      KneeLeft: left,
      Bottom: middle,
      KneeRight: right,
        ...
        scales: {
          x: {
            ...
            time: {
              ...
              tooltipFormat: 'HH:mm',
              min: new Date().setHours(0, 0, 0, 0), // 👈 เริ่มต้นที่ 00:00 ของวันนั้น
              max: new Date().setHours(23, 59, 59, 999) // 👈 ไปถึง 23:59
            },
            ticks: {
              autoskip: false,
