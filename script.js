  ...
  function loadAndListenHistory() {
    const historyRef = firebase.database().ref("history");
      ...
      function addPointToChart(sensor, value, time) {
        ...
        if (sensor === 'KneeLeft') {
          if (shouldAddPoint(kneeLeftData, timestamp)) {
        ...
        if (sensor === 'KneeRight') {
          if (shouldAddPoint(kneeRightData, timestamp)) {
      ...
      function addPoint(sensor, value, time) {
        addPointToChart(sensor, value, time); // วาดกราฟเหมือนเดิม
        updateCount(sensor, value); // นับเฉพาะตอนนี้
      }
      ...
      historyRef.once('value').then(snapshot => {
        ...
        Object.entries(data).forEach(([_, entry]) => {
          ...
          if (entry.KneeLeft !== undefined) {
            allKneeLeft.push({ x: timestamp, y: entry.KneeLeft });

            const mmHg = Math.round((entry.KneeLeft / 31.25) * 10) / 10;
            updateCount('KneeLeft', mmHg);
          }
          if (entry.Bottom !== undefined) {
            allBottom.push({ x: timestamp, y: entry.Bottom });

            const mmHg = Math.round((entry.Bottom / 31.25) * 10) / 10;
            updateCount('Bottom', mmHg);
          }
          if (entry.KneeRight !== undefined) {
            allKneeRight.push({ x: timestamp, y: entry.KneeRight });

            const mmHg = Math.round((entry.KneeRight / 31.25) * 10) / 10;
            updateCount('KneeRight', mmHg);
          }
        });
        ...
        filterEvery10Seconds(allKneeLeft).forEach(p => {
          updateCount('KneeLeft', p.y);
          addPointToChart('KneeLeft', p.y, new Date(p.x));
        });
        filterEvery10Seconds(allBottom).forEach(p => {
          updateCount('Bottom', p.y);
          addPointToChart('Bottom', p.y, new Date(p.x));
        });
        filterEvery10Seconds(allKneeRight).forEach(p => {
          updateCount('KneeRight', p.y);
          addPointToChart('KneeRight', p.y, new Date(p.x));
        });
      ...
      historyRef.on('child_added', snapshot => {
        ...
        const leftValue = entry.KneeLeft !== undefined ? Math.round((entry.KneeLeft / 31.25) * 10) / 10 : undefined;
        const middleValue = entry.Bottom !== undefined ? Math.round((entry.Bottom / 31.25) * 10) / 10 : undefined;
        const rightValue = entry.KneeRight !== undefined ? Math.round((entry.KneeRight / 31.25) * 10) / 10 : undefined;

        if (leftValue !== undefined) tryAdd('KneeLeft', leftValue, time);
...
// โชว์ tooltip ตามตำแหน่งเมาส์
  ...
  // ตั้งค่าช่วงค่าที่จะสุ่ม
    const minRaw = 500;
    const maxRaw = 4000;

  // สร้าง wave pattern โดยใช้ Math.sin โดยมีความถี่ และแอมพลิจูดปรับได้
    const amplitude = (maxRaw - minRaw) / 2;   // ความกว้างของคลื่น
    const base = minRaw + amplitude;            // จุดกึ่งกลางของคลื่น

  // speed ปรับความเร็วของ wave ให้เปลี่ยนค่า
    const speed = 0.1;

  // คำนวณค่าจาก wave pattern สำหรับแต่ละ sensor (เลื่อนเฟสให้ไม่ซ้ำกัน)
    const leftValue = base + amplitude * Math.sin(timeCounter * speed);
    const middleValue = base + amplitude * Math.sin(timeCounter * speed + Math.PI / 3); // เลื่อนเฟส 60 องศา
    const rightValue = base + amplitude * Math.sin(timeCounter * speed + (2 * Math.PI) / 3); // เลื่อนเฟส 120 องศา

  // สร้างข้อมูลจำลอง
    const simulatedData = {
      left: Math.round(leftValue),
      middle: Math.round(middleValue),
