        ...
        scales: {
          x: {
            ...
            ticks: {
              ...
              source: 'linear',
              autoSkip: false,
              maxTicksLimit: 144, // แสดงสูงสุด 24 ชั่วโมง
    ...
    historyRef.once('value').then(snapshot => {
      ...
      function filterEvery10Seconds(data) {
        ...
        for (const point of sortedData) {
          if (point.x >= anchor) {
            result.push(point);
            anchor = point.x + 10 * 1000; // เพิ่ม 10 วินาที
          }
        }
        return result;
      }
      ...
      [...filteredKneeLeft].forEach(point => {
        if (point.y > 31.9 && point.y <= 70) pressureCounts.KneeLeft.yellow++;
        else if (point.y > 70) pressureCounts.KneeLeft.red++;
      });
      [...filteredBottom].forEach(point => {
        if (point.y > 31.9 && point.y <= 70) pressureCounts.Bottom.yellow++;
        else if (point.y > 70) pressureCounts.Bottom.red++;
      });
      [...filteredKneeRight].forEach(point => {
        if (point.y > 31.9 && point.y <= 70) pressureCounts.KneeRight.yellow++;
        else if (point.y > 70) pressureCounts.KneeRight.red++;
      });
      
      document.getElementById("count-left-yellow").textContent = pressureCounts.KneeLeft.yellow;
      document.getElementById("count-left-red").textContent = pressureCounts.KneeLeft.red;
      document.getElementById("count-bottom-yellow").textContent = pressureCounts.Bottom.yellow;
      document.getElementById("count-bottom-red").textContent = pressureCounts.Bottom.red;
      document.getElementById("count-right-yellow").textContent = pressureCounts.KneeRight.yellow;
      document.getElementById("count-right-red").textContent = pressureCounts.KneeRight.red;
